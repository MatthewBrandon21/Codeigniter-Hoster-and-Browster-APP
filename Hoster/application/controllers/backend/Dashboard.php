...
class Dashboard extends CI_Controller {
	...
	public function index(){
		$data['booking'] = $this->db->query("select * from booking,hotel,user where booking_hotel=hotel_id and booking_user=user_id order by booking_id desc limit 10")->result(); //menampilkan 10 booking terakhir
        $data['user'] = $this->db->query("select * from user order by user_id desc limit 3")->result(); //menampilkan 3 user baru
        $data['hotel'] = $this->db->query("select * from hotel order by hotel_id desc limit 3")->result(); //menampilkan 3 hotel baru
    ...
    function ganti_password_act(){
        ...
        if($this->form_validation->run() != false){
            ...
            $this->m_booking->update_data($w,$data,'admin');
	...
	function hotel(){
		...
		$this->load->view('backend/dashboard/hotel', $data);
	}
	function hotel_add(){
		$data['page_title']       = 'Add Hotel';
		$this->load->view('backend/dashboard/hotel_add', $data);
	}
	function hotel_add_act(){
		...
		$foto = $this-> $this->m_booking->_uploadImageHotel();
		...
		if($this->form_validation->run() != false){
			...
			$this->load->view('backend/dashboard/hotel_add');
		}
	}
	...
	function booking(){
		...
		$data['booking'] = $this->db->query("select * from booking,hotel,user where booking_hotel=hotel_id and booking_user=user_id")->result();
		$this->load->view('backend/dashboard/booking', $data);
	}
	function booking_add(){
		$data['page_title']       = 'Add reservation';
		$data['hotel'] = $this->m_booking->get_data('hotel')->result();
		$data['user'] = $this->m_booking->get_data('user')->result();
		$this->load->view('backend/dashboard/booking_add', $data);
	}
	function booking_add_act(){
		...
		if($this->form_validation->run() != false){
			...
			redirect(base_url().'backend/dashboard/booking');
		} else {
			//$data['hotel'] = $this->m_booking->get_data('hotel')->result();
			//$data['user'] = $this->m_booking->get_data('user')->result();
			$this->load->view('backend/dashboard/booking_add', $data);
	...
	function booking_hapus($id){
		...
		$data2   = $this->m_booking->edit_data($w2,'hotel')->row();
		$jmlkmrhtl = $data2->hotel_kamar;
		$data3  = array('hotel_kamar' => $jmlkmrhtl + $jmlkamar);

		$this->m_booking->update_data($w2,$data3,'hotel');
